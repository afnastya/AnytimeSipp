...
bool Check(const char* filename) {
    ...
    for (uint32_t i = 1; i < data.path.size(); ++i) {
        ...
        if (distance != 0 && distance != path[i].time - path[i - 1].time) {
            std::cout << "incorrect representation of path " << PathSegment{path[i - 1], path[i]} << " ";
            return false;
        }
    ...
    for (uint32_t i = 1; i < data.path.size(); ++i) {
        ...
        if (PathsIntersect(path_segment, data.obstacles, true)) {
            std::cout << "path intersects obstacle path" << " ";
            return false;
        }
    }
...
int main(int argc, char **argv) {
    ...
    for (const auto& dir_entry : std::filesystem::directory_iterator{logs_directory}) {
        ...
        try {
            bool result;
            std::cout << filename << "   \t" << ((result = Check(filename.c_str())) ? "correct" : "") << std::endl;
            if (!result) {
                return 1;
            }
