...
```
logs_directory - директория с файлами, полученными на выходе при работе программы SippSearch. Чекер проверяет полученные результаты, в правильном ли виде они представлены, не пересекается ли траектория с препятствиями.
## Визулизация
```bash
